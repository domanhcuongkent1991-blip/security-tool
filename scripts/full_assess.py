...
def scan_network(target: str, out: Path, profile: str) -> Step:
    s = _skill_path("network-scanner", "scripts", "nmap_scan.py")
    nmap_out = out / "nmap"
...
def recon_web(url: str, out: Path, idx: int) -> Step:
    s = _skill_path("web-app-scanner", "scripts", "web_recon.py")
...
def main() -> int:
    ...
    if kind in ("artifact", "jsmap"):
        ...
        s = _skill_path("windows-log-hunter", "scripts", "hunt_eventlog.py")
