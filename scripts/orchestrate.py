...
def _skill_path(*parts: str) -> Path:
    """Resolve a skill script path in both layouts: skills/<name> (plugin) or <name> (repo)."""
    name = parts[0]
    for base in (REPO_ROOT / "skills", REPO_ROOT):
...
def fingerprint_binary(target: Path) -> tuple[PhaseResult, dict]:
    ...
    script = _skill_path("binary-identifier", "scripts", "identify_app.py")
...
def looks_like_electron_source(target: Path) -> bool:
    """Return True for an already-unpacked Electron app source tree.

    Distinguishes a developer/source layout (package.json declaring an
    "electron" dependency, typically with main.js) from a packed install
    (resources/app.asar), which choose_breach_skill() routes to the unpacker.
    """
    ...
    try:
        ...
        return False
    deps = {**(meta.get("dependencies") or {}), **(meta.get("devDependencies") or {})}
    if "electron" not in deps and not str(meta.get("name", "")).lower().startswith("electron"):
        return False
    return bool(meta.get("main") or (target / "main.js").is_file() or (target / "index.js").is_file())


# --- Phase runners -----------------------------------------------------------

def run_electron_unpack(target: Path, out_dir: Path) -> PhaseResult:
    script = _skill_path("electron-builder-unpacker", "scripts", "unpack_electron_builder.py")
...
def run_electron_analyze(source_dir: Path, out_dir: Path) -> PhaseResult:
    script = _skill_path("electron-app-analyzer", "scripts", "analyze_electron.py")
...
def run_js_extract(url_or_path: str, out_dir: Path) -> PhaseResult:
    script = _skill_path("javascript-deobfuscator", "scripts", "extract_sourcemap.py")
...
def run_ida_nuitka_reconstruct(target: Path, out_dir: Path) -> PhaseResult:
    ...
    extract_script = _skill_path("ida-nuitka-reconstructor", "scripts", "extract_rsrc_strings.py")
    ...
    recon_script = _skill_path("ida-nuitka-reconstructor", "scripts", "reconstruct_nuitka_source.py")
...
def run_dotnet_decompile(target: Path, out_dir: Path) -> PhaseResult:
    ...
    decompile_script = _skill_path("dotnet-decompiler", "scripts", "decompile_dotnet.py")
    ...
    analyze_script = _skill_path("dotnet-decompiler", "scripts", "analyze_dotnet.py")
...
def run_pyinstaller_unpack(target: Path, out_dir: Path) -> PhaseResult:
    ...
    unpack_script = _skill_path("pyinstaller-unpacker", "scripts", "unpack_pyinstaller.py")
    ...
    decompile_script = _skill_path("pyinstaller-unpacker", "scripts", "decompile_pyc.py")
...
def run_rust_analyze(target: Path, out_dir: Path) -> PhaseResult:
    ...
    script = _skill_path("rust-binary-analyzer", "scripts", "analyze_rust.py")
...
def run_tauri_unpack(target: Path, out_dir: Path) -> PhaseResult:
    ...
    unpack_script = _skill_path("tauri-unpacker", "scripts", "unpack_tauri.py")
    ...
    if assets_dir.exists() and any(assets_dir.rglob("*")):
        analyze_script = _skill_path("tauri-unpacker", "scripts", "analyze_tauri.py")
...
def run_java_decompile(target: Path, out_dir: Path) -> PhaseResult:
    ...
    # Step 1: Decompile
    decompile_script = _skill_path("java-decompiler", "scripts", "decompile_java.py")
    ...
    if src_dir.exists() and any(src_dir.rglob("*.java")):
        analyze_script = _skill_path("java-decompiler", "scripts", "analyze_java.py")
...
def run_android_analyze(target: Path, out_dir: Path) -> PhaseResult:
    ...
    script = _skill_path("android-apk-pentester", "scripts", "analyze_apk.py")
    phase = make_phase(
...
def run_network_analyze(target: Path, out_dir: Path, har_file: str | None = None) -> PhaseResult:
    ...
    analyze_script = _skill_path("network-interceptor", "scripts", "analyze_traffic.py")
...
def run_memory_scan(dump_file: str, out_dir: Path) -> PhaseResult:
    ...
    scan_script = _skill_path("memory-dumper", "scripts", "scan_memory.py")
...
def run_find_patch_targets(source_dir: Path, out_dir: Path) -> PhaseResult:
    ...
    script = _skill_path("dotnet-patcher", "scripts", "find_patch_targets.py")
...
def run_audit_generic(source_dir: Path, out_dir: Path) -> PhaseResult:
    ...
    script = _skill_path("dotnet-decompiler", "scripts", "analyze_dotnet.py")
...
def run_license_audit(source_dir: Path, out_dir: Path) -> PhaseResult:
    ...
    script = _skill_path("license-robustness-audit", "scripts", "audit_license.py")
...
def run_container_cloud_audit(target: Path, out_dir: Path) -> PhaseResult:
    ...
    script = _skill_path("container-cloud-auditor", "scripts", "analyze_container_cloud.py")
    phase = make_phase(
...
def run_supply_chain_audit(target: Path, out_dir: Path) -> PhaseResult:
    ...
    script = _skill_path("sbom-supply-chain-auditor", "scripts", "analyze_supply_chain.py")
    phase = make_phase(
        "audit:supply-chain",
...
def run_nuitka_extract(target: Path, out_dir: Path) -> PhaseResult:
    script = _skill_path("nuitka-decryptor", "scripts", "analyze_binary.py")
...
def execute_mission(
    ...
    # Phase 2.5 - Direct analysis of an already-unpacked Electron source tree.
    # choose_breach_skill() only routes packed bundles (*.asar/app.asar) to the
    # unpacker; a pre-unpacked app folder (package.json + main.js, no .asar) is
    # handed straight to the analyzer so it is not silently skipped.
    electron_unpacked = out_dir / "electron-unpacked"
    electron_source = None
    if not electron_unpacked.exists() and target_path and target_path.is_dir():
        if looks_like_electron_source(target_path):
            electron_source = target_path
    ...
    electron_target = electron_unpacked if electron_unpacked.exists() else electron_source
    if electron_target:
        report.phases.append(run_electron_analyze(electron_target, out_dir))
...
def _support_footer_enabled() -> bool:
    """The donation footer stays on by default; set REPORT_NO_SUPPORT_FOOTER=1 to omit it."""
    return os.environ.get("REPORT_NO_SUPPORT_FOOTER", "").strip().lower() not in ("1", "true", "yes")
...
def main() -> int:
    ...
    report = execute_mission(
        args.target, out_dir,
        include_remediation_note=True, include_report_package_note=True,
    )
    write_report(report, out_dir)

    failed = [p for p in report.phases if p.returncode != 0]
    executed = [p for p in report.phases if not p.noop]
    noop = [p for p in report.phases if p.noop]
    exec_failed = [p for p in executed if p.returncode != 0]
    print(
        f"[+] Phases executed: {len(executed)} (failed: {len(exec_failed)})"
        f" | advisory/no-match: {len(noop)}"
    )
    if not executed:
        print("[!] No analysis phase actually ran for this target "
              "(only advisory/no-match notes). See REPORT.md.", file=sys.stderr)
