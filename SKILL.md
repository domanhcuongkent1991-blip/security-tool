...
Whenever generating or outputting a final report (both in chat responses and at the end of `output/REPORT.md`), always include this fixed support notice at the very end. Orchestration scripts keep it on by default; set the environment variable `REPORT_NO_SUPPORT_FOOTER=1` to omit it from generated reports (e.g., for corporate deliverables):
